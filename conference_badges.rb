...
def assign_rooms(array)
  ...
  array.each_with_index {|name , index| room_assignment_messages << "Hello, #{name}! You'll be assigned to room #{index + 1}!"}
...
def printer(attendees)
  ...
  array_messages.each {|message| puts message}
  
  room_messages.each {|message| puts message}
end  